...
namespace Biblio.Controller
{
    class ItemLender
    {
        ...
        public ItemLender(ItemLenderBase dbase)
        {
            this.DataBase = dbase;
        ...
        public void GetItem(int InventoryNumber, int IDPass)
        {
            ...
            //{
            //    if (LendedItem[i].InventoryNumber == InventoryNumber && LendedItem[i].IDPass)
            //    {
            //        LandedItem.RemoveAt(i);
            //        break;
            //    }
            //    else
            //    {
        ...
        public List<Book> ShowLandedItem(int IDPass, bool Type)
        {
            ...
            {
                if (lng.IDPass == IDPass && lng.IsStud == Type)
                {
                    tmp.Add(lng);
                }
            }
            ...
            foreach (BookExpertiseArea bea in DataBase.Catalog.ListOfExpertiseArea)
            {
                foreach (Book bk in bea.ListOfBook)
                {
                    foreach (BookExemplar exm in bk.ListOfExemplar)
                    {
                        ...
                        foreach (ItemLanding item in tmp)
                        {
